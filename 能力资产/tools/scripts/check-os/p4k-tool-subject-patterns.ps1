$script:P4kToolSubjectChecks = @(
    ...
    @{ Path = "操作系统/02_智能体/产品PM-需求拆解者.md"; Pattern = 'Docs/2-技术文档|Claude Code\s*战场|Codex\s*战场|操作系统 PM\s*战场|`tools/\*\*`|__CZXT_APP_REPO_DIR_REGEX__/__tests__|→ Claude Code（外部）|Claude Code 实施|执行载体当前为 Claude Code|当前执行载体：Claude Code'; Label = "产品 PM playbook 仍有旧路径或工具战场责任表述" },
    @{ Path = "操作系统/02_智能体/产品PM-需求拆解者-附录.md"; Pattern = 'Docs/2-技术文档|Claude Code\s*战场|Codex\s*战场|操作系统 PM\s*战场|`tools/\*\*`|__CZXT_APP_REPO_DIR_REGEX__/__tests__|→ Claude Code（外部）|Claude Code 实施|执行载体当前为 Claude Code|当前执行载体：Claude Code'; Label = "产品 PM 附录仍有旧路径或工具战场责任表述" },
...
foreach ($check in $script:P4kToolSubjectChecks) {
