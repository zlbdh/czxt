...
foreach ($caseFile in @(
    'borrowing-capture-transaction-success-cases.ps1',
    'borrowing-capture-transaction-failure-cases.ps1'
  )) {
  Invoke-CzxtContract ('transaction semantic case passes: ' + $caseFile) {
    $path = Join-Path $PSScriptRoot $caseFile
    Assert-CzxtTrue (Test-Path -LiteralPath $path -PathType Leaf) `
      ('missing transaction case: ' + $caseFile)
